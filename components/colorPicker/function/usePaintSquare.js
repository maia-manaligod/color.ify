...
export const useSaturation = (canvas, hue) => {
    useEffect(() => {
        const ctx = canvas.current.getContext('2d')
        ctx.fillStyle = `hsl(${hue}, 100%, 50%)`
        ctx.rect(0, 0, squareSize, barSize)
        const gradientBlack = ctx.createLinearGradient(0, 0, squareSize, 0)
        gradientBlack.addColorStop(0, `rgba(127, 127, 127, 1)`)
        gradientBlack.addColorStop(1, `hsl(${hue}, 100%, 50%)`)
        ctx.fillStyle = gradientBlack
        ctx.fill()
    }, [canvas])
}
