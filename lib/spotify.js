...
export async function getProfile(){
    console.log("is this thing evern reaching the spotify getprofile")
    const access = cookieStore.get('access');
    console.log('access @ getProfile', access, access.value)
    ...
    if (response.status == 401){
        ...
        if (refresh.success){
            ...
            let response2 = await fetch ("https://api.spotify.com/v1/me" ,{
                headers: {
                    Authorization : `Bearer ${access.value}`
                },
            });
            ...
            if (response2){
                console.log("response after refresh: ", response2, refresh.success)
...
export async function refreshToken(){
    ...
    //old refresh: 
    //AQBKS-xbDpomrWI2h05l7Zt2MMEvle0bqRHbl_4cA72mVilNm720kPud8j_lh4UHserMKxo6nu59YB0pUQQ1hawTpT675mRaLI1_x3qif-F1LU7lqiUHlF6jaiKaSlzVwRo
    ...
    console.log("result from refresh:", result.status)
    if (result.status == 200){
        console.log(result)
        let data = await result.json()
        console.log("data:", data)
        console.log("refresh", data.refresh_token)
        let accessT = data.access_token
        let refreshT = data.refresh_token

        if (accessT){
            cookieStore.delete("access")
            cookieStore.set("access", accessT)
        }
        if (refreshT){
            cookieStore.delete("refresh")
            cookieStore.set("refresh", refreshT)
        }
        return {success: true}
    } else {
